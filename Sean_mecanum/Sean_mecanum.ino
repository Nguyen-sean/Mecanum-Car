...
void loop() {
  ...
  read_line_Black();
    Serial.print("động cơ trái: ");
  Serial.print(sensors[0]);
  Serial.print("động cơ phải: ");
  ...
  if (Mode_change_IS_line_follow) {
    ...
    if (Gamepad.Get_status_button_90D_Right()) {
      Radius_speed_L = constrain(map(Gamepad.Get_RAD_Joy_L(), 0, 255, 30, 70), 0, 100);
      DEG_joystick_L = constrain(Gamepad.Get_DEG_Joy_L(), 0, 360);
      Radius_speed_R = constrain(map(Gamepad.Get_RAD_Joy_R(), 0, 255, 30, 60), 0, 100);
      DEG_joystick_R = constrain(Gamepad.Get_DEG_Joy_R(), 0, 360);
    } else {
      Radius_speed_L = constrain(map(Gamepad.Get_RAD_Joy_L(), 0, 255, 30, 100), 0, 100);
      DEG_joystick_L = constrain(Gamepad.Get_DEG_Joy_L(), 0, 360);
      Radius_speed_R = constrain(map(Gamepad.Get_RAD_Joy_R(), 0, 255, 30, 80), 0, 100);
      DEG_joystick_R = constrain(Gamepad.Get_DEG_Joy_R(), 0, 360);
    }
    ...
    if (Gamepad.Get_RAD_Joy_R() > 10) {
      ...
      if ((DEG_joystick_L >= 315) || (DEG_joystick_L < 45)) {
        ...
        rotateClockwise(Radius_speed_L);
        // right
        ...
        // Serial.println("F");
        rotateCounterClockwise(Radius_speed_L);
        // forward();
